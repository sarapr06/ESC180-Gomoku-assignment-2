...
Assume that the sequence is complete (i.e., you are not just given a subsequence) and valid, and contains stones of only one colour.'''
    ...
    for i in range(length):
        ...
        coords.append(x_end-d_x*i)
        L.append(coords)
    if (x_end ==7 and d_x==1) or (y_end==0 and d_y == -1) or (y_end ==7 and d_y == 1) or (x_end == 0 and (d_x==-1)):
        if (L[len(L)-1][0]==7 and d_y==-1) or (L[len(L)-1][0]==0 and d_y == 1) or (L[len(L)-1][1] == 7 and d_x==-1) or (L[len(L)-1][1]==0 and d_x == 1):
            return "CLOSED"
    ...
    elif (L[len(L)-1][0]==7 and d_y==-1) or (L[len(L)-1][0]==0 and d_y == 1) or (L[len(L)-1][1] == 7 and d_x==-1) or (L[len(L)-1][1]==0 and d_x == 1):
        if (x_end ==7 and d_x==-1) or (y_end==0 and d_y == 1) or (y_end ==7 and d_y == -1) or (x_end == 0 and (d_x==1)):
            return "CLOSED"
    ...
    #
        ...
        if y_end+d_y>=8 or x_end+d_x>=8 or L[len(L)-1][0]-d_y<0 or L[len(L)-1][1]-d_x<0:
            return "SEMIOPEN"
        return "OPEN"
    #
...
on the board. The only situation where "Draw" is returned is when board is full'''
    ...
    options = ["White won", "Black won", "Draw", "Continue playing"]
    while len(empty_slots)!=0:
        if detect_rows(board, "w", 5):
            return "White won"
        elif detect_rows(board, "b", 5):
            return "Black won"
