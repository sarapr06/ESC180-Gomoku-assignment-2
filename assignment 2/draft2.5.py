...
Assume that the sequence is complete (i.e., you are not just given a subsequence) and valid, and contains stones of only one colour.'''
    ...
    if x_end + d_x ==8 or x_end+d_x<0 or y_end + d_y ==8 or y_end+d_y<0:
        if L[0][0]-d_y==8 or L[0][0]- d_y <0 or L[0][1] - d_x==8 or L[0][1] -d_x<0:
            return "CLOSED"
        elif board[L[0][0]-d_y][L[0][1]-d_x] != " ":
            return "CLOSED"
        else:
            return "SEMIOPEN"
    #
    elif L[0][0]-d_y==8 or L[0][0]- d_y <0 or L[0][1] - d_x==8 or L[0][1] -d_x<0:
        if x_end + d_x ==8 or x_end+d_x<0 or y_end + d_y ==8 or y_end+d_y<0:
            return "CLOSED"
        elif board[L[0][0]-d_y][L[0][1]-d_x] != " ":
            return "CLOSED"
    ...
    elif board[y_end+d_y][x_end+d_x] == " " and board[L[0][0]-d_y][L[0][1]-d_x] == " ":
        if y_end+d_y==8 or x_end+d_x==8 or L[0][0]-d_y<0 or L[0][1]-d_x<0:
            return "SEMIOPEN"
        return "OPEN"
    #

    elif board[y_end+d_y][x_end+d_x] == " " or board[L[0][0]-d_y][L[0][1]-d_x] == " ":
            return "SEMIOPEN"
    #
...
def is_col(col, y_end, board, x_end, d_y, d_x, length): #done
    ...
    for i in range(length):
        coords = []
        if y_end - d_y*i==8 or y_end - d_y*i<0 or x_end - d_x*i==8 or x_end - d_x*i<0:
            return False
        coords.append(y_end - d_y*i)
        coords.append(x_end-d_x*i)
        L.append(coords)
...
'''
    ...
    for i in range(len(board)):
        seqs=[]
        if y_start+d_y*i>=8 or x_start+d_x*i>=8:
            break
    ...
    if (is_bounded(board, seq[0][0], seq[0][1], length, d_y, d_x) == "OPEN") and is_col(col, seq[0][0],  board, seq[0][1], d_y, d_x, length):
        open_seq_count+=1
    elif (is_bounded(board, seq[0][0], seq[0][1], length, d_y, d_x) == "SEMIOPEN") and is_col(col, seq[0][0],  board, seq[0][1], d_y, d_x, length):
